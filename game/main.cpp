...
public:
  ...
  void reset() {
    ...
    btAlignedObjectArray<btRigidBody *> &bodies = dynamics.getNonStaticRigidBodies();

    for (int i = 0; i < bodies.size(); ++i) {
      btRigidBody *body = bodies.at(i);

      body->setAngularVelocity(zeroVector);
      body->setLinearVelocity(zeroVector);
      body->clearForces();
      body->setGravity({0, -10, 0});
    }
...
public:
  ...
  void setHorizontalPosition(const float horizontal_position) {
    if (!enabled) {
      auto current_pos = world.falling_sphere.body.getCenterOfMassPosition();
      btTransform transform;
      transform.setIdentity();
      transform.setOrigin(btVector3(horizontal_position, current_pos.y(), current_pos.z()));
      world.falling_sphere.body.setCenterOfMassTransform(transform);
    }
  }

  void setVerticalPosition(const float vertical_position) {
    if (!enabled) {
      auto current_pos = world.falling_sphere.body.getCenterOfMassPosition();
      btTransform transform;
      transform.setIdentity();
      transform.setOrigin(btVector3(current_pos.x(), vertical_position, current_pos.z()));
      world.falling_sphere.body.setCenterOfMassTransform(transform);
    }
  }
  ...
  void start() {
    ...
    bool running = true;
    float horizontal_position = 0.0;
    float vertical_position = 0.0;
    ...
    while (running) {
      ...
      while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::KeyPressed) {
          if (sf::Keyboard::isKeyPressed(sf::Keyboard::Escape)) {
            window.close();
            running = false;
          } else if (sf::Keyboard::isKeyPressed(sf::Keyboard::R)) { // Added keyboard shortcuts
            onRestart();
          } else if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) {
            onDropBall();
          } else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
            setHorizontalPosition(horizontal_position - 0.1);
          } else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) {
            setHorizontalPosition(horizontal_position + 0.1);
          } else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
            setVerticalPosition(vertical_position + 0.1);
      ...
      horizontal_position = world.falling_sphere.body.getCenterOfMassPosition().x();
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, -3.0, 3.0)) {
        setHorizontalPosition(horizontal_position);
      }
      vertical_position = world.falling_sphere.body.getCenterOfMassPosition().y();
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &vertical_position, 0.5, 3.5)) {
        setVerticalPosition(vertical_position);
      }
      ImGui::End();
